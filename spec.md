...
Portrait Coach Web — a browser-native portrait analysis tool served at `app.gyatso.me/onframe/web/`. No install required. Works in Safari on iPhone, Chrome on Android, and any desktop browser. Produces the same 6-category coaching output as the iOS app.

**Privacy guarantee (both modes):**
- Photo is immediately drawn to a `<canvas>` element — EXIF metadata (GPS, device info, timestamps) is stripped in this step
- All analysis (face detection, lighting, sharpness) runs on canvas pixel data, never on the original file
- After analysis: canvas cleared via `ctx.clearRect`, `<img>` src set to `''`, blob URL revoked via `URL.revokeObjectURL`
- No image data written to localStorage, sessionStorage, IndexedDB, cache storage, or any cookie
- No image pixels ever sent to any server
- Cloud mode sends only a ~300-character text string of measured numbers to the proxy — no pixels
...
[Browser: VisionMetrics object]       ← text/numbers only, no pixels
      │
      ├── Local mode ──→ [synthesizer.js]
      │                    Rules-based, JS port of iOS FeedbackSynthesizer
      │
      └── Cloud mode ──→ [POST /onframe/web/api/analyze]
                           body: { summary: "Face detected: true\nHead yaw: 18.3°..." }
                               │
                    [web-server/server.js: Express proxy, port 3004]
                    GROQ_API_KEY env var · no body logging · no storage
                               │
                    [Groq API: llama-3.3-70b-versatile]
                               │
                    [Structured JSON: 6 FeedbackCards + aiSummary]
      │
      ▼
...
| Cloud AI | Groq `llama-3.3-70b-versatile` | Open-source (Meta Llama 3.3), free tier 14K req/day, <1s, JSON mode |
...
### 1.8 Cloud Mode — Groq Proxy

**Client (`cloud.js`):**
```javascript
async function fetchCloudCoaching(metrics) {
  ...
  return res.json();  // { cards, overallScore, aiSummary }
}
```

**Server (`web-server/server.js`) — Express on port 3004:**
```
POST /onframe/web/api/analyze
  Validate: body.summary string, 10–5000 chars
  Rate limit: 10 req/min per IP (express-rate-limit)
  Log: timestamp + status code only — no request body logged
  Send to Groq: model=llama-3.3-70b-versatile, response_format={type:"json_object"}
  System prompt: portrait coaching expert; same knowledge base as iOS FoundationModelsService
  Parse response: same FeedbackCard JSON schema as iOS
  Return: { cards, overallScore, aiSummary }
  503 if GROQ_API_KEY missing; 503 on Groq timeout; 500 on parse failure
```

**Env vars (Cloud Run):**
```
GROQ_API_KEY=gsk_...            required for cloud mode
AI_PROVIDER=groq                default; set to 'anthropic' to use Claude
AI_MODEL=llama-3.3-70b-versatile  overridable
PORT=3004
```
...
**Mode toggle (top-right, persisted to localStorage):**
```
[ 🔒 Local · Private ]   [ ☁️ Cloud · Open-source AI ]
```
...
| State | Display |
|-------|---------|
| `idle` | Upload zone (drag-drop + file picker + camera on mobile); privacy badge |
| `analyzing` | Photo dimmed + centered spinner + "Analyzing locally…" label |
| `analyzing-cloud` | Same + label changes to "Asking AI…" after local phase |
| `complete` | Photo thumbnail + score + 6 coaching cards (worst expanded, great collapsed) |
| `error` | Friendly message (no face / blurry / network failure) |

**Responsive layout:**
- Mobile (< 640px): single column; cards stacked below photo
- Desktop (≥ 640px): two-column — photo + score left (sticky), cards scroll right
...
├── app.js              State machine; wires file picker, drag-drop, camera, mode toggle
├── analysis.js         loadMediaPipe(), analyzeImage(file) → VisionMetrics
├── synthesizer.js      Rules-based coaching → { cards, overallScore }
├── cloud.js            fetchCloudCoaching(metrics) → { cards, overallScore, aiSummary }
└── models/             Created at Docker build time
    └── face_landmarker.task  (2.4MB, self-hosted)

onframe/web-server/
├── package.json        { express, express-rate-limit }
└── server.js           Groq proxy, port 3004
```
...
environment=PORT="3004",NODE_ENV="production"
autostart=true
autorestart=true
...
1. `docker build` succeeds; `face_landmarker.task` present in image (~2.4MB)
2. `/onframe/web/` loads on Safari iOS — upload zone visible, no console errors
3. Local mode: pick a portrait → 6 coaching cards render with scores
4. Cloud mode (GROQ_API_KEY set): richer tip text from Llama 3.3
5. Network tab: only `POST /api/analyze` in cloud mode; body is plain text; no base64/blob
6. Storage tab: no image data in localStorage/sessionStorage/IndexedDB/CacheStorage after analysis
7. No GROQ_API_KEY → HTTP 503; key not present anywhere in response body
8. POST body > 8KB → nginx returns 413 before reaching Node
9. `/onframe/` still redirects to `/onframe/ios/`
